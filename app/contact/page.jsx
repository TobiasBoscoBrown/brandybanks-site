...
export default async function Contact() {
  ...
  const links = [
    ...
    { label: 'ShopMy', href: s.shopmy },
    ...(c.site.email ? [{ label: 'Email', href: `mailto:${c.site.email}` }] : []),
  ].filter(l => l.href);
  const support = [
      ...
      <section className="block page-head">
        <div className="wrap">
          ...
          <div className="hero-actions" style={{ marginTop: 30 }}>
            ...
            {c.site.email ? <EmailButton email={c.site.email} subject="Booking Inquiry" className="btn btn-ghost" label={c.site.email} /> : null}
            {b.phone ? <a href={`tel:${b.phone}`} className="btn btn-ghost">Call</a> : null}
            {!b.url && s.instagram ? <a href={s.instagram} target="_blank" rel="noopener" className="btn btn-primary">DM on Instagram</a> : null}
          </div>
        </div>
      </section>
